...
void BWHtmlNav::init(BWConfig &cfg) {
    ...
    m_menu.FromFile(BW_HTML_NAVBAR);
    m_header.FromFile(BW_HTML_NAVBAR_HEADER);
    m_list.FromFile(BW_HTML_NAVBAR_LIST);
    m_item.FromFile(BW_HTML_NAVBAR_ITEM);
...
BWString BWHtmlNav::genItemDropdown(const BWString &it, const BWString &prefix) {
	...
	dropdown.FromFile(BW_HTML_NAVBAR_DROPDOWN);
